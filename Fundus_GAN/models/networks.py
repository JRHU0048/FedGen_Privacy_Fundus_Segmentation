...
def get_filter(filt_size=3): 
    ...
    filt = torch.Tensor(a[:, None] * a[None, :])  # The outer product generates a two-dimensional matrix
    filt = filt / torch.sum(filt)   # Ensure that the sum of the filters is 1 and keep the overall brightness of the image unchanged
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        n_downsampling = 2  # Perform n downsampling, with the number of downsampling channels doubling each time
        ...
        mult = 2 ** n_downsampling   # It contains n blocks of Resnetblocks, each block consisting of two convolutional layers with skip connections in the middle
